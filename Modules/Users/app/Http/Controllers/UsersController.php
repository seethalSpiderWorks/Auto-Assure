...
class UsersController extends Controller
{
    ...
    public function index()
	{
		...
		$privilage = PrivilegeModel::select('id','privilege_name')->where('status',0);
					if($privilege != 2)
					{
		...
		$branch = BranchModel::select('branch_id','branch_name')->where('branch_status',0);
				if($privilege != 2)
				{
	...
	public function getuserDatatable(request $request) 
	{
		...
		$user = DB::table('users')->select('users.id as userid','users.user_id','users.username','users.name','users.lname','users.previlage','users.user_branch','users.user_email','users.mobile','privilege.privilege_name','tbl_branch.branch_name','b.name as addedbyname')
	...
	public function add_users(request $request)
	{
		...
		$user_name = strtolower($request->user_name); //$user_fname = strtolower($request->user_fname);
		$username  = str_replace(' ', '', $user_name);
		
		// check username already exist in db  //
		if(UsersModel::where('username','=',$request->user_name)->exists()) 
		{  
	...
	{  
		...
		else 
		{
			...
			return response()->json(['status' => 1, 'msg' => 'User Updated successfully!', 'heading' => 'Success']);
		}
		try 
		{
		} 
