...
{	
	...
	public function getDatatable(Request $request)
	{
		...
		$area = DB::table('tbl_report')
			->select('report_id','report_reference_no','report_client_name','report_date_of_inspection','report_vehicle_plate_no','report_expired_status','users.name','tbl_lead.lead_id','tbl_lead.lead_followup_type','lead_assigned_status')
            ...
            ->Where(function($query) use ($search) 
                    {
						...
						$query->orwhere('report_vehicle_plate_no', 'like', $search . '%');
						$query->orwhere('name', 'like', $search . '%');
						$query->orWhereRaw("DATE_FORMAT(report_date_of_inspection, '%d-%m-%Y') like ?", [$search . '%']);
                    });
	...
	public function addGeneralInfo(Request $request)
    {
        ...
        $vehicle_plate_no   = $request->report_vehicle_plate_no;
        ...
        $data = array(
				...
				'report_vehicle_plate_no'  => $vehicle_plate_no, 	
                );
         
		if($edit_id == '' || $edit_id == null)
		{   
		    $data['report_addedby'] = Auth::user()->id;
	...
	public function add_additionalSpec(Request $request)
    {
		...
		if($update_id == '' || $update_id == null)
		{
		    $data['add_spec_addedby'] = Auth::user()->id;
	...
	public function add_warrantyServices(Request $request)
    {
		...
		if($update_id == '' || $update_id == null)
		{
		    $data['war_service_addedby'] = Auth::user()->id;
    ...
    {   
		...
		if($update_id == '' || $update_id == null)
		{
		    $data['overview_addedby'] = Auth::user()->id;
	...
	public function add_inspectionChecklist(Request $request)
	{
		...
		if($update_id == '' || $update_id == null)
		{
			...
			$data_one['one_checklist_addedby'] = Auth::user()->id;
			$insertid_one = DB::table('tbl_report_insp_check_one')->insertGetId($data_one);
			
			$data_two['two_checklist_addedby'] = Auth::user()->id;
	...
	public function add_inspectionGallery(Request $request)
    {
				...
				if ($count_int > 0) {
					for ($i = 0; $i < $count_int; $i++) {
						...
						if (!in_array($extension, ['jpg', 'jpeg', 'png'])) {
							return response()->json(['status' => 0, 'msg' => 'Please select a valid image file!', 'heading' => 'Error']);
						}

						// Validate file size (max 2MB)
						if ($cv->getSize() > 2000000) {
							return response()->json(['status' => 0, 'msg' => 'Please select an image less than 2MB!', 'heading' => 'Error']);
						}

						$file_name = Str::random(30) . '.' . $extension;
						$uploadPath = public_path('uploads/inspectionreport/gallery/');

						$destinationPath = $uploadPath . $file_name;

						// ---- Create GD image resource ----
						if ($extension === 'jpeg' || $extension === 'jpg') {
							$source = imagecreatefromjpeg($cv->getRealPath());
						} elseif ($extension === 'png') {
							$source = imagecreatefrompng($cv->getRealPath());
						} else {
							continue;
						}

						// ---- Resize large images ----
						list($width, $height) = getimagesize($cv->getRealPath());
						$maxWidth  = 1200;
						$maxHeight = 1200;
						...
						if ($extension === 'jpeg' || $extension === 'jpg') {
							...
							$pngQuality = 8; // 0=best compression, 9=worst
							imagepng($source, $destinationPath, $pngQuality);
						}

						imagedestroy($source);

						// ---- Insert into DB ----
						DB::table('tbl_report_gallery')->insert([
							'gallery_report_id'    => $edit_id,
							'gallery_status'       => '0',
							'gallery_addedby'      => Auth::user()->id,
							'gallery_ip'           => $request->ip(),
							'gallery_image'        => $file_name,
							'gallery_image_type'   => $request->gallery_image_type[$i] ?? null,
							'gallery_image_desc'   => $request->gallery_image_desc[$i] ?? null,
						]);
					}
				}
			}
		...
		else 
		{
			...
			// Step 1: Mark old gallery images as inactive
			DB::table('tbl_report_gallery')
			...
			// Step 3: Re-insert previously selected images (no compression needed)
			if (!empty($request->docimg)) {
				for ($i=0; $i<count($request->docimg); $i++) 
				{
					$d1 = $request->docimg[$i];  
					$previousData = DB::table('tbl_report_gallery')->where('gallery_id', $d1)->first();
					if ($previousData) 
					{
						$inData1 = array(
								'gallery_ip'        => $request->ip(),
								...
								'gallery_image_type'=> $previousData->gallery_image_type, 
								'gallery_image_desc'=> $previousData->gallery_image_desc, 
							);    
			...
			if ($request->hasFile('file')) {
				...
				for ($i = 0; $i < $count_int; $i++) {
					...
					// Validate file type
					if (!in_array($extension, ['jpg', 'jpeg', 'png'])) {
						return response()->json(['status' => 0, 'msg' => 'Please select a valid File!', 'heading' => 'Error']);
					}

					// Validate file size (max 2MB)
					if ($cv->getSize() > 2000000) {
						return response()->json(['status' => 0, 'msg' => 'Please select an image less than 2 MB!', 'heading' => 'Error']);
					}
					...
					if ($extension === 'jpeg' || $extension === 'jpg') {
						...
						$pngQuality = 8; // Higher = smaller file
						imagepng($source, $destinationPath, $pngQuality);
					}
					imagedestroy($source);
	...
	/********** Gallery END **********/
	/********** Gallery 24/10/2025 **********/
	public function add_inspectionGallery_OLD(Request $request)
    {
        $edit_id   = $request->edit_id; /** report_id for insert other tables **/   
        $update_id = $request->update_id; /** report_id for edit **/    
    ...
    {
		$edit_id   = $request->editid; /** report_id for insert other tables **/   
        $update_id = $request->update_id; /** report_id for edit **/    
	...
	public function getDivisionAction(Request $request)
    {
        ...
        $report = DB::table('tbl_report')
			->select('report_id','report_reference_no','report_client_name','report_client_name_ar','report_date_of_inspection','report_vehicle_plate_no','users.user_branch')
			->leftjoin('users', 'users.id' ,'=','tbl_report.report_addedby')
		...
		// Fetch the existing record (if exists)
		$record = DB::table('watheeq_inspection_answers')
			->select('answers_id', 'answers_report_id', 'answers')
			//->where('answers_user_id', $user_id)
			->where('answers_report_id', $divisionId)
			->first();   

		// Decode the answers JSON (safe decode)
		$answers = [];
    ...
    public function SummaryDes(Request $request)
	{
	    $data = DB::table('tbl_summary_description')
			...
			->orderBy('sum_desc_name', 'asc')->pluck('sum_desc_name','sum_desc_id'); 
    ...
    public function storeChecklist(Request $request)
	{
		...
		DB::table('watheeq_inspection_answers')->updateOrInsert(
			[
				'answers_status'    => 0,
				'answers_user_id'   => auth()->id(),
				'answers_ip'        => $request->ip(),
				'answers_report_id' => $request->edit_id,
			],
			[
		...
		$insertid = $request->edit_id; 
		
		if($insertid)
		{
			return response()->json(['status'=>1, 'msg'=>'Checklist saved successfully', 'heading'=>'Success', 'id'=>$insertid]);
		}
		else
		{
			return response()->json(['status' => 0, 'msg' => 'Error !try again', 'heading' => 'Error']);
