...
<script type="text/javascript">
  ...
  var url_setfilter_campaign = "{{url('/leads/setFilterCampaign')}}";
		...
		<script src="{{asset('module.js/Leads/index.js?ver=2.5')}}"></script>  
...
$(document).ready(function() {
    ...
    // Get selected model for edit
    let selectedModel = $('#model').attr('data-selected-model');
            ...
            success: function(res) {
                ...
                if (res) {
                    $.each(res, function(id, name) {
                        let selected = (id == selectedModel) ? 'selected' : ''; 
    ...
    $('#make').on('change', function() {
        selectedModel = ''; // Clear previous selection if user changes Make
        loadModels($(this).val());
    });
});
