  ...
  //{
	...
	"columns": 
            [
				...
				{"data": "breg_id", "name": "breg_id"},
				{"data": "breg_id", "name": "breg_id"},
				{"data": "inspection_id", "name": "inspection_id", "defaultContent": "", "orderable": false},
				{"data": "breg_id", "name": "breg_id"},
            ],
	"fnCreatedRow": function (nRow, aData, iDataIndex) 
    {
		...
		// Inspection column: link to the inspection page + date assigned.
		var inspection = '<span class="text-muted">&mdash;</span>';
		if(aData.inspection_id)
		{
			var assignedDate = '';
			if(aData.inspection_assigned_at)
			{
				var idt = new Date(aData.inspection_assigned_at.replace(' ', 'T'));
				if(!isNaN(idt.getTime()))
				{
					assignedDate = ('0'+idt.getDate()).slice(-2)+'-'+('0'+(idt.getMonth()+1)).slice(-2)+'-'+idt.getFullYear();
				}
			}
    	...
    	$('td:eq(11)', nRow).html(action).addClass('center');
    }
});
...
function confirmEdit_new(id)
{
	$.ajax({
            ...
            success: function(result)
            {
				...
				$("#whatsapp").val(result.data.breg_whatsapp);
				...
				//console.log(result.data.lead_form_type);
				...
				$('#lead_vehicle_plate_no').val(result.data.lead_vehicle_plate_no);
				...
				$('#make_model_year').val(result.data.make_model_year); 
