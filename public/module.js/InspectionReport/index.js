	...
	$.ajax({
       type: 'POST',
		...
		{ 
			...
			if(result.report.user_branch == 39)
			{  
				console.log(result.report.user_branch);
				...
				if (result.report.user_branch == 39) {
					$("#checklist-tab, #specification-tab, #specification, #checklist").hide();
					var checklistType = 'saudi-checklist';
				} else {
					$("#checklist-tab, #specification-tab, #specification, #checklist").show();

					var checklistType = 'specification';
				}

				// Remove old click handlers to prevent duplicates
				$('#saveNextButton').off('click').on('click', function (event) {
					event.preventDefault();
					insertEntry('https://crm.auto-assure.com/inspectionreport/addVehicleInfo',
						'vehicleForm','add-modal','inspectionReportDataTable', '', true, checklistType);
				});
			}
			else{
				$("#saudi-checklist-tab, #saudi-checklist").hide();
			}
            ...
            // Loop through answers and pre-fill
            ...
            // Set hidden fields
            //$('.edit_id').val(result.record?.id || '');
            //$('#report_id').val(result.record?.id || '');
			//toastr.success('Checklist loaded for editing.');
		...
		if (result.checklistTwo) {	
			...
			for(var i=0;i<int_count;i++)
			{
				...
				$('#img_g'+new_row_data).attr("src","https://crm.auto-assure.com/public/uploads/inspectionreport/gallery/"+result.gallery[i].gallery_image);
			...
			for(var i=0;i<int_count;i++)
			{
				...
				$('#img_f'+new_row_data).attr("src","https://crm.auto-assure.com/public/img/icons/pdf.png");
				$('#img_fi'+new_row_data).attr("href","https://crm.auto-assure.com/public/uploads/inspectionreport/reports/"+result.repfile[i].rep_file);
			...
			/*********** DAMAGED IMAGE START ***********/
			if (result.damageImg && result.damageImg.damage_image) {
				var img = document.createElement("img");
				img.src = "https://crm.auto-assure.com/public/uploads/inspectionreport/damages/"+result.damageImg.damage_image;
